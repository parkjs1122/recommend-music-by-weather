...
for n in range(N_CLUSTERS):
    idx_cluster = np.where(kmeans == n)[0]
    for i in idx_cluster:
        f.write("%d "%(i))
    f.write("\n")
    print("Cluster %d : size = %d"%(n, len(idx_cluster)))
    n_cluster_users = np.take(ex_train_user, idx_cluster)
    ...
    for u in n_cluster_test_user:
        ...
        if len(idx_cluster) < 8000:
            ...
            rank = sorted(zip(music_idx, scores), key=lambda x: -x[1])
